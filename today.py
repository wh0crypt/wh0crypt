...
#
# Issues and pull requests permissions not needed at the moment, but may be used
# in the future
HEADERS = {"authorization": "token " + os.environ["ACCESS_TOKEN"]}
USER_NAME = os.environ["USER_NAME"]  # 'wh0crypt'
QUERY_COUNT = {
    "user_getter": 0,
    "follower_getter": 0,
    "graph_repos_stars": 0,
...
def simple_request(func_name: str, query: str, variables: Dict[str, Any]) -> Response:
    """
...
def graph_repos_stars(
    ...
    """
    Uses GitHub's GraphQL v4 API to return my total repository, star, or
    lines of code count.
    """
...
def recursive_loc(
    ...
    repo_name: str,
    data: List[str],
    cache_comment: Union[List[str], str],
...
query ($repo_name: String!, $owner: String!, $cursor: String) {
    ...
    # Cannot use simple_request() since we want to save
    # the file before raising Exception
    request = post(
        "https://api.github.com/graphql",
        json={"query": query, "variables": variables},
        headers=HEADERS,
...
def loc_query(
    ...
    (with respect to owner_affiliation)
    Queries 60 repos at a time, because larger queries give a 502 timeout error
...
def cache_builder(
    ...
    """
    Checks each repository in edges to see if it has been updated since
    ...
    for index in range(len(edges)):
        ...
        if repo_hash == check_hash.hexdigest():
            try:
                count = edges[index]["node"]["defaultBranchRef"]["target"]["history"][
                    "totalCount"
                ]
                if int(commit_count) != count:
...
def flush_cache(edges: List[Dict], filename: str, comment_size: int) -> None:
    ...
    This is called when the number of repositories changes or when
...
def add_archive() -> List[Union[int, float]]:
    """
...
def force_close_file(data: List[str], cache_comment: Union[List[str], str]) -> None:
    """
    Forces the file to close, preserving whatever data was written to it.

    This is needed because if this function is called, the program would've crashed
    before the file is properly saved and closed.
    """
...
def svg_overwrite(
    ...
    """
    Parse SVG files and update elements with my age, commits, stars, repositories,
    and lines written.
    """
    ...
    justify_format(root, "loc_data", str(loc_data[2]), 9)
    justify_format(root, "loc_add", str(loc_data[0]))
    justify_format(root, "loc_del", str(loc_data[1]), 7)
    tree.write(filename, encoding="utf-8", xml_declaration=True)


def justify_format(
    root: etree._ElementTree, element_id: str, new_text: str, length: int = 0
) -> None:
    """
...
def perf_counter(funct: Callable[..., Any], *args: Any) -> Tuple[Any, float]:
    """
...
def formatter(
    query_type: str, difference: float, funct_return: bool = False, whitespace: int = 0
) -> Union[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    # e.g {'id': 'U_kgDOBsvW3A'}
    # and 2022-09-20T18:49:25Z for username 'wh0crypt'
    ...
    # only calculate for user wh0crypt
    if OWNER_ID == {"id": "U_kgDOBsvW3A"}:
    ...
    # and the total function time, then move cursor back
    print(
